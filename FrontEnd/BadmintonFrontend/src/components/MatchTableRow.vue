                ...
                <div v-show="isVisible && !containsDetailedScores">
                    <table class="w-full">
                        <thead class="bg-gray-200">
                            <tr>
                                <th colspan="2" class="py-1 text-center font-semibold text-gray-800">Männer {{ computedRegisteredMans }}/4</th>
                                <th colspan="2" class="py-1 text-center font-semibold text-gray-800">Frauen {{ computedRegisteredWoman }}/2</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr v-for="(player, playerIndx) in (playerStore.getPlayersTableformats[props.gameIndx])" :key="playerIndx" class="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                                ...
                                <td> <div v-if="player.woman !== '|LAST|' && player.woman  !== ''">{{ playerIndx + 1 }}.</div> </td>
                                <td v-if="player.woman !== '|LAST|' && player.woman  !== ''" @mouseover="hoveredLine = playerIndx; hoverMan = false;" @mouseleave="hoveredLine = -1">
                                    {{ player.woman }}
                                ...
                                <td v-if="player.woman === '|LAST|'" class="space-x-1">
...
<script setup>
    ...
    const gameColorClass = computed(
        () => {
            ...
            if (computedRegisteredMans.value >= 4 && computedRegisteredWoman.value >= 2)
